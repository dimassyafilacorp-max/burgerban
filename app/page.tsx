...
export default function Home() {
  ...
  const hasBundlingInCart = useMemo(() => {
    ...
    return cart.some(
      (item) =>
        ((item.category as string) === 'paket' || (item.category as string) === 'combo') &&
        item.quantity > 0
    );
  }, [cart, isMounted]);
    ...
    try {
      ...
      try {
        const { error } = await supabase.from('regular_orders').insert([
          {
            order_id: orderId,
            nama: customerName.trim() || 'Pelanggan Ritel',
            telepon: '-',
            items: itemsForDb,
        ...
        console.warn('Gagal koneksi ke Supabase, melanjutkan alur checkout IG:', dbErr);
      }

      // 2. Format Teks Pesanan untuk DM Instagram
      ...
      const message =
        `Halo Burgerban! Saya mau konfirmasi pesanan Pre-Order:\n\n` +
        `ID Pesanan: ${orderId}\n` +
      ...
      // 3. Salin Teks Pesanan ke Clipboard
      try {
      ...
      // 4. Clear state, tutup keranjang, lalu buka Modal Popup Sukses UI/UX Kustom
      setCart([]);
      setCustomerName('');
      setAddress('');
      setNotes('');
  ...
  // Handler Konfirmasi Modal untuk Membuka DM Instagram
  const handleProceedToInstagram = () => {
    setIsSuccessModalOpen(false);
    const instagramUsername = 'burgerbanbwi';
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 font-sans relative flex flex-col justify-between selection:bg-amber-400 overflow-x-hidden">
              ...
              {cart.length > 0 && (
                <div className="p-6 border-t border-gray-200 bg-gray-50 space-y-4">
                  ...
                  <button
                    ...
                    disabled={!hasBundlingInCart || isSubmitting}
                    className={`w-full font-bold py-3.5 px-4 rounded-xl transition flex items-center justify-center gap-2 shadow-md cursor-pointer touch-manipulation ${
                      hasBundlingInCart && !isSubmitting
                        ? 'bg-gradient-to-r from-purple-600 via-pink-500 to-red-500 hover:opacity-90 text-white active:scale-98'
      ...
      {/* Modal Popup UI/UX Sukses Disalin */}
      {isSuccessModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-sm w-full text-center shadow-2xl border border-gray-100 flex flex-col items-center">
            {/* Animated Check Icon */}
            ...
            <p className="text-gray-600 text-sm leading-relaxed mb-6">
              Rincian pesanan berhasil disalin! Silakan <strong>Paste (Tempel)</strong> di DM Instagram kami (<strong>@burgerbanbwi</strong>).
            </p>

            <button
              type="button"
              onClick={handleProceedToInstagram}
              className="w-full bg-gradient-to-r from-purple-600 via-pink-500 to-red-500 hover:opacity-95 text-white font-bold py-3.5 px-6 rounded-2xl shadow-lg transition duration-200 active:scale-95 flex items-center justify-center gap-2 cursor-pointer touch-manipulation"
            >
              Lanjut ke DM Instagram
            </button>
          </div>
        </div>
      )}
